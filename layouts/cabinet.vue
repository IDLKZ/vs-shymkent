        ...
        <div class="container">
          ...
          <div class="account__tabs">
            <ul class="account__tabs-caption">
              <NuxtLink :style="'color:'+tab.color" :to="tab.link" v-for="tab in tabs" :key="tab.id" :class="tab.active" @click.native="activeTab(tab.id)" v-html="tab.svg+tab.title"></NuxtLink>
              <a @click="logout">
...
export default {
  ...
  created() {
    if (this.$auth.user.user.role_id === 6 || this.$auth.user.user.role_id === 7){
      this.tabs[4].title = 'Создать товар'
